...
class FixedSpreadSignalGenerator(StatArbSignalGenerator):
    def __init__(self,
                 ...
                 dailyExitTime=None):
        super(FixedSpreadSignalGenerator, self).__init__(startRunningTime,
                                                         orderRatio,
                                                         timeInterval,
                                                         orderWaitingTime,
                                                         dataLogFixedTimeWindow,
        ...
        while True:
            ...
            if in_or_out == 0:
                ...
                if self.maximum_qty_multiplier is not None:
                    Qty = min(Qty, max(self.huobi_min_quantity,
                                       self.okcoin_min_quantity) * self.maximum_qty_multiplier)
                ...
                if Qty < self.huobi_min_quantity or Qty < self.okcoin_min_quantity:
                    self.timeLog("当前在火币的币量：%.4f，OKCoin的现金：%.2f" % (
                        accountInfo[helper.coinTypeStructure[self.coinMarketType]
                                    ["huobi"]["coin_str"]],
