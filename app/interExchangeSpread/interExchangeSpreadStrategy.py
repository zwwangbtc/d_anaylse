...
class BanZhuanStrategy(object):
    ...
    def getAccuntInfo(self):
        ...
        okcoin_cny_cash_frozen = float(
            okcoinAcct["info"]["funds"]["freezed"]["cny"])
        okcoin_cny_btc_frozen = float(
            okcoinAcct["info"]["funds"]["freezed"]["btc"])
        okcoin_cny_ltc_frozen = float(
            okcoinAcct["info"]["funds"]["freezed"]["ltc"])
            ...
            if huobi_buy_1_price > okcoin_sell_1_price:  # 获利信号：OKcoin买，huobi卖
                ...
                if Qty < self.huobi_min_quantity or Qty < self.okcoin_min_quantity:
                    ...
                    if executed_qty is not None:
                        # step2: 再执行买
                        Qty2 = min(executed_qty, Qty)
                        self.buy(self.coinMarketType, str(
