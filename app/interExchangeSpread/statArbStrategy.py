...
class StatArbSignalGenerator(object):
    def __init__(self, startRunningTime, orderRatio, timeInterval, orderWaitingTime, dataLogFixedTimeWindow,
        ...
        # 1: long spread1(buy okcoin, sell huobi);  2: long spread2( buy huobi, sell okcoin), 0: no position
        self.current_position_direction = 0
        self.spread1_pos_qty = 0
        self.spread2_pos_qty = 0
        # if position proportion > position_proportion_alert_threshold, generate alerts
        self.position_proportion_alert_threshold = 0.80
        # if position proportion > position_proportion_threshold, stop upsizing, only allow downsizing
        self.position_proportion_threshold = 0.90
        self.spread_pos_qty_minimum_size = max(
            self.okcoin_min_quantity, self.huobi_min_quantity) * 1.5
    ...
    def getStartRunningTime(self):
        if self.startRunningTime == None:
            raise StartRunningTimeEmptyError(
                "startRunningTime is not set yet!")
    ...
    def rebalance_position(self, accountInfo, price):
        ...
        target_okcoin_pos_value = accountInfo["okcoin_cny_net"] * \
            self.rebalanced_position_proportion
    ...
    def calc_sma_and_deviation(self):
        self.spread1_mean = np.mean(
            self.spread1List[-1 * self.sma_window_size:])
        self.spread1_stdev = np.std(
            self.spread1List[-1 * self.sma_window_size:])
        self.spread2_mean = np.mean(
            self.spread2List[-1 * self.sma_window_size:])
        self.spread2_stdev = np.std(
            self.spread2List[-1 * self.sma_window_size:])
    ...
    def in_or_out(self):
        if self.current_position_direction == 0:  # currently no spread position
            ...
            # spread1 close is using the price data of spread2, so check spread2 here
            ...
            # spread2 close is using the price data of spread1, so check spread1 here
