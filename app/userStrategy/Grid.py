...
def handle_data(context):
    ...
    if context.user_data.base_price is None:
        ...
        hist = context.data.get_price(
            context.security, count=context.user_data.sma_window_size, frequency="1d")
    ...
    if cash_to_spent > 0:
        ...
        #  计算需要卖出的数量，并已市价单卖出
        quantity = min(
            getattr(context.account, context.security), -1 * cash_to_spent / price)
        context.log.info("正在卖出 %s" % context.security)
