...
def handle_data(context):
    ...
    # 2 判断加仓或止损
    # 先判断是否持仓
    if context.user_data.hold_flag is True and getattr(context.account, context.security) > 0:
        temp = add_or_stop(
        ...
        if out == 1:  # 入场
            if context.user_data.hold_flag is False:
                value = min(context.account.huobi_cny_net *
                            0.01, context.account.huobi_cny_cash)
